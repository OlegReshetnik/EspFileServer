...
static const char f_err_space[] PROGMEM = "Нет места для обновления";
...
void EspFileServer::_handleUpdate(void)
{
	...
	if( upload.status == UPLOAD_FILE_START )
	{
		...
		if( !Update.begin(maxSketchSpace) ) { _result = F_ERR_SPACE; }
	}
...
void EspFileServer::_handleUpdatePost(void)
{
	if( Update.hasError() )
	{
		...
		switch( _result )
		{
			case F_ERR_SPACE:	str = f_err_space; break;
			case F_ERR_WRITE:	str = f_err_write; break;
			case F_ERR_END:		str = f_err_end; break;
			case F_ERR_ABORT:	str = f_err_abort; break;
		}
		_server->send_P( 200, text_plain, str );
	}
	else
	{
